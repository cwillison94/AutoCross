...
class LaneDetector:
    ...
    def base_distance(self, x1, y1, x2, y2):
    ...
    def detect(self, frame):
        ...
        #start_canny = timeit.default_timer()

        contours_for_lane = cv2.Canny(blur_for_lane, 50, 200)

        #canny_time = timeit.default_timer() - start_canny
        #print "Canny time = \t",canny_time

        if self.prob_hough:

            #start_hough = timeit.default_timer()
            ...
            #hough_time = timeit.default_timer() - start_hough
        ...
        if lines is not None:
            ...
            #TODO: Figure out what this line does and remove if it is not necessary
            lines = lines+np.array([0, self.lane_roi, 0, self.lane_roi]).reshape((1, 1, 4))  # scale points from ROI coordinates to full frame coordinates
            left_bound = None
            right_bound = None
            left_lane = None
            right_lane = None

            start_lane_find = timeit.default_timer()
